...
global Round RN CorrectNum
    ...
    Round = Round + 1;
    set(handles.text3,'String',['第' num2str(Round) '次猜數字']);
    ...
    for i = 1:4
        ...
        if(Index == i) % 如果剛好在符合位置 且有找到
            A = A + 1;
        elseif (Index ~= i) % 如果不符合位置 但有找到
            B = B + 1;
        end
    end
    if A == 4 % 有4個A就結束
        set(handles.text2,'String','正確答案');
